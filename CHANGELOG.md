...
### Added
- `test/regression/contract-host-fixtures.js` / `test/regression/DETERMINISM_BASELINE.json`: added an `inline:attestation-request-paid` determinism fixture that calls `xchain.attestation.request()` with non-empty `feeTick`/`feeAmount` options, pinning the paid-attestation emission path distinctly from the existing zero-fee `inline:attestation-request` fixture. The fee fields are pass-through strings the VM only shape-checks (the indexer enforces the consensus rules), so without a non-zero-fee fixture a regression in fee normalisation or `ATTEST` emission shaping on the paid path would shift no pinned digest. Its digest differs from the zero-fee fixture's, confirming the fee fields actually reach the hashed emission. All other pre-existing digests are byte-identical.
...
### Changed
- `src/gateway-emit.js`: `xchain.emit.crossExecute` now derives the cross-chain-call `call_id` from `network:chain:txHash:contractIndex:emissionIndex:targetChain`, dropping the emitting EXECUTE's `actionIndex` from the preimage. `actionIndex` depends on the indexer's synthetic-action injection timing rather than on chain content, so binding it in would make `call_id` non-deterministic across nodes after any one-block injection slip (and, because the synthetic XEXEC/callback tx-hashes are derived from `call_id`, that divergence would compound and never re-converge); `(txHash, contractIndex, emissionIndex)` already identify the emission uniquely. This MUST stay byte-identical to the indexer's re-derivation in `xchain-indexer/src/actions/xcall.js`; both sites changed in the same release. `test/unit/emit-crossexecute.test.js`, the regression fixture comment in `test/regression/contract-host-fixtures.js`, and the pinned `inline:emit-crossexecute` digest in `test/regression/DETERMINISM_BASELINE.json` are updated to the six-component preimage (only that one digest changed; all other baselines are byte-identical).
...
### Fixed
- `src/index.js`, `test/security/allocation-gas-metering.test.js`, `test/determinism/consensus-params.test.js`, `test/regression/binary-alloc-gate.regression.test.js`: the binary-allocation gas charge (the F3-binary `ArrayBuffer`/`TypedArray` byte-length metering injected by the isolate harness) is now gated on a coordinated block-time flag-day (`BINARY_ALLOC_GATE_BLOCK_TIME`, `1798761600`: the same activation timestamp as the indexer's other 2.0.0 protocol changes) instead of firing unconditionally on every block. That byte-length charge is consensus-affecting: `gasUsed` is hashed into the per-block contracts checkpoint and drives the fee debit, so a node that applies the charge and a node that does not produce a different `gasUsed`: and can produce opposite `success` vs `out_of_gas` outcomes near the gas ceiling, for the same binary-allocating execution. Applied to all blocks (including historical ones) the change therefore forks any mixed-version fleet on the first contract that constructs a `TypedArray`/`ArrayBuffer` by length. The harness now charges the byte length only when the execution's block time is at/after the flag day, the host injects that block time (the value the indexer already threads through as `blockContext.timestamp`) into the isolate alongside the flag-day constant before the harness runs; below the flag day the constructors are left unmetered, exactly as pre-activation nodes leave them, so the whole fleet flips the rule atomically at one timestamp rather than whenever an individual node happens to upgrade. A missing/garbage timestamp resolves below the flag day, so an un-timestamped embedder can never accidentally enable the new charge. The flag-day constant is exported and pinned by the consensus-params freeze guard (a divergent value is itself a fork), and a new regression fixture locks both schedule versions: it asserts the identical allocation yields a different `gasUsed` below vs at the flag day, that the at/after charge is byte-proportional, and that an allocation which `out_of_gas`-reverts at the flag day succeeds below it. The Array/String F3 metering and every other gas charge are unchanged.
