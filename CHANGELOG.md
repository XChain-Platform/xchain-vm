...
- CHANGELOG cites the correct armed flag-day timestamp for ASYNC_SURFACE_GATE_BLOCK_TIME ().
...
- Confirmed `ASYNC_SURFACE_GATE_BLOCK_TIME` and `BINARY_ALLOC_GATE_BLOCK_TIME` at the armed contract-era flag-day 1786060800 (2026-08-07 00:00 UTC), in lockstep with the indexer.
...
### Fixed
- `src/index.js`: gate the binary-allocation gas charge (`ArrayBuffer`/`TypedArray` byte-length metering) behind `BINARY_ALLOC_GATE_BLOCK_TIME` (since armed at the coordinated 2.0.0 flag-day `1786060800`) so the whole fleet flips the rule atomically at one timestamp.
