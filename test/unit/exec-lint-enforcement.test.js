    ...
    describe('verdict cache keyed by the metering sha256 key', function () {
        ...
        it('returns the verdict a fresh validateSyntax call produces', function () {
            ...
            for (const code of [CLEAN, GENERATOR, ASYNC]) {
                const cached = vm._getLintVerdict(code, true, true, true);
        ...
        it('partitions on the three consensus flag bits, not just the source', function () {
            const vm = newVm();
            vm._getLintVerdict(GENERATOR, false, false, false);
            vm._getLintVerdict(GENERATOR, true,  false, false);
            vm._getLintVerdict(GENERATOR, false, true,  false);
            vm._getLintVerdict(GENERATOR, false, false, true);
            assert.strictEqual(vm._lintVerdictCache.size, 4);
            ...
            assert.strictEqual(vm._getLintVerdict(GENERATOR, false, false, false).valid, true);
            assert.strictEqual(vm._getLintVerdict(GENERATOR, false, false, true).valid, false);
        });
        ...
        it('accepts the shared sha256 digest and keys identically to computing it itself', function () {
            ...
            vm._getLintVerdict(CLEAN, true, true, true, hash);
            vm._getLintVerdict(CLEAN, true, true, true);
        ...
        it('evicts FIFO at the bound instead of growing without limit', function () {
            ...
            vm._getLintVerdict(CLEAN,     true, true, true);
            vm._getLintVerdict(GENERATOR, true, true, true);
            vm._getLintVerdict(ASYNC,     true, true, true);
            ...
            assert.strictEqual(vm._getLintVerdict(CLEAN, true, true, true).valid, true);
        });
    });
