...
describe('execute-time consensus source-lint enforcement @regression @tier1', function () {
    ...
    describe('lint gas is metered', function () {
        ...
        it('charges VM_COMPUTATION per EXEC_LINT_GAS_BYTES_PER_UNIT bytes of source', async function () {
            ...
            const padLen = 4096;
            const padded = CLEAN + '\n//' + 'x'.repeat(padLen);
            assert.strictEqual(vm.getMeteredCode(CLEAN, false, false),
                vm.getMeteredCode(padded, false, false),
...
describe('execute-time consensus source-lint enforcement @regression @tier1', function () {
    ...
    describe('verdict cache keyed by the metering sha256 key', function () {
        ...
        it('partitions on the five consensus flag bits, not just the source', function () {
            ...
            assert.strictEqual(vm.getLintVerdict(GENERATOR, false, false, true, false, false).valid, false);
            ...
            assert.strictEqual(vm.getLintVerdict(ALIASED, true, true, true, false, false).valid, true);
            assert.strictEqual(vm.getLintVerdict(ALIASED, true, true, true, true, false).valid, false);
...
describe('execute-time consensus source-lint enforcement @regression @tier1', function () {
    ...
    describe('verdict cache keyed by the metering sha256 key', function () {
        ...
        it('evicts FIFO at the bound instead of growing without limit', function () {
            ...
            vm.getLintVerdict(CLEAN,     true, true, true, true, true);
            vm.getLintVerdict(GENERATOR, true, true, true, true, true);
            vm.getLintVerdict(ASYNC,     true, true, true, true, true);
